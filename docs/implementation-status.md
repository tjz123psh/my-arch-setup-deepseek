...
| Fixed AUR pipeline | Three VM-selected recipes passed real source/build/artifact/install/rerun paths, including partial-chroot recovery | Twelve recipes are enumerated behind the independent production-readiness gate/private-cache prerequisites |
...
| Full ASUS policy | Plan-visible and fail-closed with 21 production module blockers | Physical hardware acceptance and 12 recipe paths remain unproved |
...
2. The 12 non-VM-selected AUR recipes and every production-planning module
   remain blocked by the independent readiness manifest; no VM subset result is
   generalized to them. This includes `flclash-bin` even though its reviewed
   autostart config surface remains registry-available.

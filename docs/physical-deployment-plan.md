...
Status: **APPROVED-SCOPE — decisions resolved (2026-08-04); execution pending
operator go-ahead. No apply performed yet.**
...
| Explicit packages | 183 = 170 sync + 13 AUR | matches repository inventory exactly |
...
| AUR on host | 13 | exactly equals repository AUR set — **0 difference** (claude-code and intellij-idea-ultimate-edition removed from host 2026-08-04, D1) |
| repo inventory | 183 | 170 sync + 13 AUR |
| repo policy rows | 203 | 183 current + 20 confirmed desired |

The host and repository are fully reconciled; no residual packages remain.
...
- **D1 — RESOLVED 2026-08-04**: the two host AUR packages no longer in the
  repository (`claude-code`, `intellij-idea-ultimate-edition`) were removed
  from the physical host with `pacman -Rn` (no dependents; 4.6 GiB released).
  Host explicit package count is now 183, exactly matching the repository
  inventory (170 sync + 13 AUR, zero difference in both directions).
- **D2 — RESOLVED 2026-08-04 (operator: one-shot)**: run all nine stages in a
  single approved session without an intermediate pause. The DAG is
  fail-closed (any stage failure stops the run) and rerun-idempotent, so a
  one-shot run still reports stage-passed events per stage and stops on
  failure.
- **D3 — ACCEPTED BY DEFAULT 2026-08-04**: the 5 host-explicit sync packages
  adopted into the policy (polkit-gnome, qemu-guest-agent, spice-vdagent,
  sysbench, ydotool) are managed targets; apply is idempotent for them (already
  installed).
- **D4 — ACCEPTED BY DEFAULT 2026-08-04**: manual-action acceptance order is
  display/GPU → audio → bluetooth → suspend/resume → ASUS controls; the
  operator records each physical-hardware-acceptance item.

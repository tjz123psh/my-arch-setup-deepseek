...
> **计数注**：上表为 2026-09-18 实测值（mappings=289）。2026-09-21 同步 `~/md` 知识库后
> mappings 增至 291（+14 文档映射 / −12 已删文档），同口径的干净轮部署数相应为
> `290 / 272 / 256`；上表各轮的 `EXIT=0` 与步骤结论不变，但那批验收对应的是 289 时期的 payload。

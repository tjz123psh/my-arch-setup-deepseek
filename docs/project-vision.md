...
3. **AUR is fully automatic.** 15 AUR targets build from their real
   upstream sources via makepkg, pinned under `third_party/aur/` (16 recipe
   trees: 15 targets + `vmware-keymaps`, which is a build-time dependency of
   ...
   recipe's dependency). For machines with no overseas access, sources can be

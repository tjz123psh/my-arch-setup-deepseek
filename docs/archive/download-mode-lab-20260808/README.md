...
- 审查快照写在本目录的 `results/`；重复运行产生的临时日志/JSON 写在被 git 忽略的
  `fixtures/tmp/`；不得写入密码、token、cookie、私钥或代理凭据。
...
```bash
./run-lab.sh                 # 当前两轮均通过
python3 tests/mirror-plan-test.py  # 41/41 PASS
```

第三步 planner 的真实只读探测结果不自动覆盖 `results/` 快照；当前样本和失败状态见
`03-mirror-plan.md`。`status=degraded` 即使使用 `--allow-degraded`，输出中的
`applyable` 仍为 false。

...
| 独立 reviewer | 本阶段请求后多次等待超时并关闭，状态 **UNAVAILABLE**；没有把 reviewer 结果当作通过证据 |
...
- 本轮 reviewer 已因多次等待超时关闭，状态为 **UNAVAILABLE**；不能记作 PASS；
...
- `results/mirror-*.json`、`results/package-range-1m.json`：镜像只读探测；
...
现在最值得做、风险最低、证据最强的第一笔主代码改动是：

> **停止 `scripts/01-mirror.sh` 默认写入 XferCommand，回到 pacman 原生并发；随后再单独治理重复同步和 AUR 源预取。**

不要把这份报告理解为已经授权修改主安装器。当前工作区只完成了独立实验和文档；若要合并，下一步应先向用户展示第一小步的 diff/dry-run，取得明确批准，再在隔离 VM 与物理机测试矩阵中验证。

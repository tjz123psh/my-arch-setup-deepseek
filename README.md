...
| 09-settings | 系统设置：locale（zh_CN）/时区（上海）/主机名/zram + fish 登录 shell + snapper 快照配置 + 录屏引擎（两机型统一 wf-recorder，编码参数"均衡"档 preset=medium/crf=18/tune=animation/aq-mode=2，Alt+F5 设置菜单可切清晰/极致共 3 档；wl-screenrec-git 已自 archlinuxcn 下架且 ffmpeg9 ABI 不兼容）+ nomacs/PNG MIME 验收 |
...
- `check-extend.sh`：提交前一键总检（增改门禁，8 节：bash 语法/shellcheck/清单一致性/配置内容语法/recipe 双向引用/secret scan/README 数字/行为测试；任一节失败禁止提交）
